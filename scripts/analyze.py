...
def write_html_dashboard(
    ...
    plot_files = [
        ...
        "nbtest_poll_tradeoff.png",
    ]
    existing_plots = [name for name in plot_files if (out_dir / name).exists()]

    confidence_lines: List[str] = []
    if confidence_path.exists():
        confidence_lines = [line.strip() for line in confidence_path.read_text(encoding="utf-8").splitlines() if line.strip()]
    ...
    for row in rows:
        mode_counts[str(row.get("mode", "unknown"))] += 1

    cards = [
        ("Total rows", str(len(rows))),
        ("Modes", ", ".join(f"{m}:{c}" for m, c in sorted(mode_counts.items())) or "n/a"),
        ("Plots generated", str(len(existing_plots))),
        ("Summary CSV", "yes" if summary_csv_path.exists() else "no"),
    ]
    ...
    confidence_html = "".join(f"<li>{html.escape(line)}</li>" for line in confidence_lines if not line.startswith("#"))
    if not confidence_html:
        confidence_html = "<li>No confidence summary available.</li>"

    plot_html = ""
    if existing_plots:
        for plot_name in existing_plots:
            plot_html += (
                f"<section class='plot'><h3>{html.escape(plot_name)}</h3>"
                f"<img src='{html.escape(plot_name)}' alt='{html.escape(plot_name)}'></section>"
            )
    else:
        plot_html = (
            "<p>No PNG plots were generated (likely missing matplotlib). "
            "Install matplotlib and rerun to enable visual plots.</p>"
        )

    table_html = ""
    if summary_rows:
        headers = ["mode", "halo", "flops_per_point", "bytes_per_point", "threads", "wait_frac", "overlap_ratio", "wait_skew", "samples"]
        head_html = "".join(f"<th>{html.escape(h)}</th>" for h in headers)
        body_html = ""
        for row in summary_rows:
            body_html += "<tr>" + "".join(f"<td>{html.escape(str(row.get(h, '')))}</td>" for h in headers) + "</tr>"
        table_html = f"<table><thead><tr>{head_html}</tr></thead><tbody>{body_html}</tbody></table>"
    else:
        table_html = "<p>No summary rows found.</p>"
...
<head>
  ...
  <title>PhaseGap Report Dashboard</title>
  <style>
    ...
    .card {{ border: 1px solid #d9d9d9; border-radius: 10px; padding: 12px; background: #fafafa; }}
    .card .k {{ font-size: 0.8rem; color: #555; }}
    .card .v {{ font-size: 1rem; font-weight: 600; margin-top: 4px; }}
    .plot {{ margin: 24px 0; }}
...
<body>
  <h1>PhaseGap Report Dashboard</h1>
  <p class="muted">Generated by scripts/analyze.py</p>
  ...
  <h2>Metric Summary (first 20 rows)</h2>
  {table_html}
</body>
</html>
"""
...
def main() -> int:
    ...
    summary_csv_path = out_dir / "metric_summary.csv"
    confidence_path = out_dir / "confidence_summary.md"
    write_metric_summary(rows, summary_csv_path)
    write_confidence_summary(rows, confidence_path)
    write_html_dashboard(out_dir, rows, confidence_path, summary_csv_path)
    ...
    print(f"[analyze] wrote {out_dir / 'dashboard.html'}")
